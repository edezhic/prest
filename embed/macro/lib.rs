...
fn embedded(
  ...
  for FileEntry { rel_path, full_canonical_path } in get_files(absolute_folder_path.clone(), &includes, &excludes) {
    match_values.insert(
      rel_path.clone(),
...
fn embed_file(_folder_path: Option<&str>, _ident: &syn::Ident, _rel_path: &str, full_canonical_path: &str) -> syn::Result<TokenStream2> {
...
fn impl_rust_embed(ast: &syn::DeriveInput) -> syn::Result<TokenStream2> {
  match ast.data {
    Data::Struct(ref data) => match data.fields {
      Fields::Unit => {}
      _ => return Err(syn::Error::new_spanned(ast, "Embed can only be derived for unit structs")),
    },
    _ => return Err(syn::Error::new_spanned(ast, "Embed can only be derived for unit structs")),
  };

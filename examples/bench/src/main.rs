...
impl AnalyticsEntry {
    pub fn new(id: u64) -> Self {
        ...
        method_stats.insert(
            "GET".to_string(),
            (fastrand::u64(100..10000), fastrand::f64() * 100.0),
        );
        method_stats.insert(
            "POST".to_string(),
            (fastrand::u64(10..1000), fastrand::f64() * 200.0),
        );
        if fastrand::bool() {
            method_stats.insert(
                "PUT".to_string(),
                (fastrand::u64(1..100), fastrand::f64() * 150.0),
            );
        }
        if fastrand::bool() {
            method_stats.insert(
                "DELETE".to_string(),
                (fastrand::u64(1..50), fastrand::f64() * 80.0),
            );
        }
...
async fn setup_benchmark() -> Result<String> {
    ...
    Ok(format!(
        "Created {} entries and {} analytics entries",
        START_TABLE_SIZE, ANALYTICS_TABLE_SIZE
    ))
}
...
async fn full_scan_benchmark() -> Result<String> {
    ...
    Ok(format!(
        ...
        - Total time: {:?}",
        avg_time,
        &scan_times[0..5.min(scan_times.len())],
        &scan_times[scan_times.len().saturating_sub(5)..],
        total_time
    ))
}
...
async fn memory_stress_benchmark() -> Result<String> {
    ...
    for i in 1..=500 {
        ...
        let entry = AnalyticsEntry {
            ...
            is_asset: false,
            metadata: (0..100)
                .map(|k| format!("large_metadata_{}_{}", i, k))
                .collect(),
        };
...
async fn concurrency_benchmark() -> Result<String> {
    ...
    Ok(format!(
        ...
        if results.is_empty() {
            ...
            results.iter().sum::<std::time::Duration>() / results.len() as u32
        },
...
async fn bottleneck_benchmark() -> Result<String> {
    ...
    Ok(format!(
        ...
        analytics.len(),
        small_reads_time,
        small_read_times.iter().sum::<std::time::Duration>() / small_read_times.len() as u32,
        analytics_during_load_time,
        if bg_read_times.len() > 10 {
            let first_10_avg = bg_read_times[..10].iter().sum::<std::time::Duration>() / 10;
            let last_10_avg = bg_read_times[bg_read_times.len() - 10..]
                .iter()
...
async fn serialization_benchmark() -> Result<String> {
    ...
    for i in 1..=100 {
        ...
        method_stats.insert(
            "GET".to_string(),
            (fastrand::u64(100..1000), fastrand::f64() * 50.0),
        );
        method_stats.insert(
            "POST".to_string(),
            (fastrand::u64(10..100), fastrand::f64() * 100.0),
        );
    ...
    let avg_small_ser = serialize_times_small.iter().sum::<std::time::Duration>()
        / serialize_times_small.len() as u32;
    let avg_large_ser = serialize_times_large.iter().sum::<std::time::Duration>()
        / serialize_times_large.len() as u32;
...
async fn allocator_benchmark() -> Result<String> {
    ...
    for i in 0..10000 {
        ...
        for j in 0..10 {
            data.insert(
                format!("key_{}_{}", i, j),
                (fastrand::u64(1..1000), fastrand::f64()),
            );
        }
    ...
    for i in 0..100 {
        ...
        for j in 0..1000 {
            data.insert(
                format!("large_key_{}_{}", i, j),
                (fastrand::u64(1..10000), fastrand::f64() * 1000.0),
            );
        }
        let serialized = into_bitcode(&data).unwrap();
    ...
    for cycle in 0..1000 {
        ...
        for i in 0..50 {
            ...
            for j in 0..20 {
                data.insert(
                    format!("frag_{}_{}", cycle, j),
                    (fastrand::u64(1..5000), fastrand::f64()),
                );
            }
...
async fn release_sim_benchmark() -> Result<String> {
    ...
    for entry in analytics {
        let methods: Vec<(String, u64, f64)> = entry
            .method_stats
            .into_iter()
...
async fn reader_saturation_benchmark() -> Result<String> {
    ...
    let task_times: Vec<std::time::Duration> =
        successful_tasks.iter().map(|(_, _, t)| *t).collect();
    let avg_task_time = if task_times.is_empty() {
        std::time::Duration::ZERO
    } else {
        task_times.iter().sum::<std::time::Duration>() / task_times.len() as u32
    };
    let max_task_time = task_times
        .iter()
        .max()
        .copied()
        .unwrap_or(std::time::Duration::ZERO);
    let min_task_time = task_times
        .iter()
        .min()
        .copied()
        .unwrap_or(std::time::Duration::ZERO);
...
async fn realistic_benchmark() -> Result<String> {
    ...
    for i in 1..=10 {
        ...
        info!(
            "Load {}: {:?} - {} methods, {} total hits",
            i, load_time, method_count, total_hits
        );
    }
...
async fn system_stress_benchmark() -> Result<String> {
    ...
    for i in 1..=20 {
        ...
        method_stats.insert(
            "GET".to_string(),
            (fastrand::u64(10..100), fastrand::f64() * 50.0),
        );
        method_stats.insert(
            "POST".to_string(),
            (fastrand::u64(1..20), fastrand::f64() * 100.0),
        );
    ...
    for i in 0..5 {
        let handle = RT.spawn(async move {
            ...
            for j in 0..20 {
                let read_start = Instant::now();
                let _entry = Entry::get_by_pkey((i * 20 + j + 1) as u64)
                    .await
                    .unwrap_or_default();
                times.push(read_start.elapsed());
                sleep(std::time::Duration::from_millis(10)).await;
    ...
    let avg_analytics_time =
        analytics_times.iter().sum::<std::time::Duration>() / analytics_times.len() as u32;
    let background_task_count: usize = background_results
        .iter()
    ...
    Ok(format!(
        ...
        - DIAGNOSIS: {}",
        cache_stress_time,
        analytics_with_cache_pressure.len(),
        memory_pressure_time,
        analytics_with_memory_pressure.len(),
        concurrent_time,
        avg_analytics_time,
...
async fn data_analysis_benchmark() -> Result<String> {
    ...
    if !existing_analytics.is_empty() {
        info!(
            "Found {} existing analytics entries, analyzing...",
        ...
        for entry in &existing_analytics {
            ...
            for (method, (hits, _)) in &entry.method_stats {
                ...
                *method_counts.entry(method.clone()).or_insert(0) += 1;
                ...
                if *hits > 10000 {
                    info!(
                        "Found high hit count: {} hits for {} {}",
                        hits, method, entry.path
                    );
                }
            }
        }
        ...
        for (path, methods) in &minimal_entries {
            ...
            for method in methods {
                method_stats.insert(
                    method.to_string(),
                    (fastrand::u64(1..10), fastrand::f64() * 50.0),
                );
            }
...
async fn scan_scaling_benchmark() -> Result<String> {
    ...
    for &target_size in &test_sizes {
        ...
        info!(
            "Database size: {} entries, scan time: {:?}",
            analytics.len(),
            scan_time
        );
    }
...
async fn production_scenario_benchmark() -> Result<String> {
    info!(
        "Simulating production: 200MB database with small analytics table but lots of other data"
    );

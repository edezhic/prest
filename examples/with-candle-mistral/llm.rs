...
pub fn init(cfg: MistralConfig) -> Result<Mistral> {
...
impl Mistral {
    ...
    fn predict(&mut self) -> Result<u32> {
        let Mistral { tokens, current_ctx, cfg, .. } = self;
    ...
    fn try_decode(&mut self) {
        let Mistral { tokens, processed, current_ctx, .. } = self;
        let processed_text = self.tokenizer.decode(&tokens[*processed..*current_ctx], true).unwrap();

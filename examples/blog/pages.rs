...
pub fn service() -> Router {
    Router::new()
        .layer(middleware::from_fn(posts))
        .layer(Htmxify::wrap(full_html))
        .layer(http_middleware::set_header::SetResponseHeaderLayer::if_not_present(header::CONTENT_TYPE, header::HeaderValue::from_static("text/html")))
}
        ...
        html data-theme="dark" {
            ...
            body { 
                header."top container" {
                    nav { 
                        ul{h3."logo"{(nav_link("/", "Prest"))}}
                        ul {
                            (nav_link("/motivations", "Motivations"))
                            (nav_link("/roadmap", "Roadmap"))
                            (nav_link("/under-the-hood", "Under the hood"))
...
fn nav_link(url: &str, text: &str) -> Markup {
    maud::html!(
        li{a href={(url)} hx-boost="true" hx-swap="innerHTML transition:true show:window:top" hx-target="main" {(text)}}
    )
}